...
const Cart = () => {
  ...
  const handleQuantityChange = async (cartID, newQuantity, stock) => {
    try {
      ...
      const token = localStorage.getItem('customerToken');
      await axios.put(`/api/cart/${cartID}`, { quantity: newQuantity }, { headers: { 'Authorization': `Bearer ${token}` } });
  ...
  const handleRemove = async () => {
    ...
    try {
      const token = localStorage.getItem('customerToken');
      await axios.delete(`/api/cart/${deletingItem}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      fetchCart();
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      const response = await axios.post('/api/user-coupon/apply', 
        { 
  ...
  const fetchAvailableCoupons = async () => {
    try {
      ...
      const [availableResponse, myCouponsResponse] = await Promise.all([
        axios.get('/api/user-coupon/available', {
          params: {
        ...
        axios.get('/api/user-coupon/my-coupons', {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        })
                            ...
                            <div className="mt-auto pt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                              ...
                              <div className="flex items-center gap-2">
                                <button
                                  ...
                                  disabled={itemQuantities[item.cartID] <= 1}
                                >
                                ...
                                <button
                                  ...
                                  disabled={itemQuantities[item.cartID] >= item.stock}
                                >
          ...
          <div className={`w-full max-w-md p-6 ${
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                className="px-6 py-2 rounded-xl font-medium text-gray-600 hover:bg-gray-100"
              >
                Hủy bỏ
              </button>
              <button
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-2 mt-3">
                          ...
                          {coupon.minimumQuantity > 1 && (
                            <span className={`inline-flex items-center px-2 py-1 rounded-lg text-xs ${
                              Array.from(selectedItems).length >= coupon.minimumQuantity
                                ...
                                {Array.isArray(coupon.appliedCategories) && coupon.appliedCategories.map((cat, idx) => (
                                  <span 
                                    ...
                                    className={`inline-flex items-center px-2 py-1 rounded-lg text-xs ${
                                      coupon.applicableCategories?.some(appliedCat => 
                      ...
                      <button
                        ...
                        className={`ml-4 px-4 py-2 rounded-xl text-sm font-medium transition-all ${
                          theme === 'tet'
